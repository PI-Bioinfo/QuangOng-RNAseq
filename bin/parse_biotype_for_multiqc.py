...
def mqc_feature_stat(counts_files, features, top_n):
    ...
    for bfile in sorted(counts_files):
        ...
        if total_count == 0:
            ...
            return
        # Try to read and parse hemoglobin featureCounts file
        if os.path.isfile(sname+'_hemoglobin.featureCounts.txt.summary'):
            try:
                with open(sname+'_hemoglobin.featureCounts.txt.summary', 'r') as hfl:
                    txt = hfl.read()
                    m = re.search(r"Assigned\t(\d+)", txt)
                    fcounts['hemoglobin'] = float(m.group(1))
                fcounts['protein_coding'] -= fcounts['hemoglobin']
            except:
                logger.error("Trouble reading the hemoglobin featureCounts summary file {}_hemoglobin.featureCounts.txt.summary".format(sname))
