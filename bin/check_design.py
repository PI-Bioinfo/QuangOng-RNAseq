...
def check_design(DesignFileIn, DesignFileOut, Comparison, Kitqcplotdatasets, IgnoreR1):
    ...
    if Kitqcplotdatasets is not None:
        ...
        with open(Kitqcplotdatasets, "r") as fin:
            # Check the header first
            header = fin.readline().strip().split(',')
            assert header == HEADER, "Header of scatterplot datasets file Incorrect! Shoulud be {}".format(','.join(HEADER))
            # Check the specified pairs of datasets
            for line in fin:
                cols = line.strip().split(',')
                # Check the number of columns in each line
                assert len(cols) == len(HEADER), "Number of columns incorrect in line '{}'!".format(line.strip())
                # Check the dataset labels
                assert cols[0] in labels, "Dataset label '{}' not in design file".format(cols[0])
                assert cols[1] in labels, "Dataset label '{}' not in design file".format(cols[1])

    subprocess.check_call(["mv", DesignFileIn, DesignFileOut])
...
if __name__ == "__main__":
    ...
    check_design(args.DesignFileIn, "checked_"+args.DesignFileIn, args.Comparison, args.Kitqcplotdatasets, args.IgnoreR1)